...
f = figure("Name", "Compound Network Link: All K's");
axes;
...
for x = 1:9
    % Iterates through each k
    for k = [1, 5, 15, 50, 100]
        % Calls on the function to get the number of transmissions
        y = runCompoundNetworkSim(k, x ./ 10, 1000);
...
function buildGraph(k)
    f = figure("Name", "Compound Network: K-" + k);
    axes;
    hold all;
